...
FUNC double shfl_down(double x, uint s){
  ...
  lo = __shfl_down_sync(FULL_MASK,lo,s);
  hi = __shfl_down_sync(FULL_MASK,hi,s);
...
FUNC float shfl_down(float x, uint s){
  return __shfl_down_sync(FULL_MASK,x,s);
} 
...
FUNC double shfl(double x, int s) {
  int lo, hi;
  asm volatile( "mov.b64 {%0,%1}, %2;" : "=r"(lo), "=r"(hi) : "d"(x) );
  lo = __shfl_sync(FULL_MASK,lo,s);
  hi = __shfl_sync(FULL_MASK,hi,s);
  asm volatile( "mov.b64 %0, {%1,%2};" : "=d"(x) : "r"(lo), "r"(hi) );
  return x;
}
...
FUNC float shfl(float x, uint s){
  return __shfl_sync(FULL_MASK,x,s);
} 
