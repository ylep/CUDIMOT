				...
				T &gradYLocal)
{
  ...
  T dParMdPerp = P[8] - Dperpendicular;
  T delta = (T)0.00005;
  ...
  dPerp2 = P[8] + 2*Dperpendicular - dPar - dPerp1;
  ////////
				...
				T &gradYLocal)
{
  ...
  T parComp =-CFP[3]*P[8]; // Parallel component: -bval * dintra
...
{
  T isoterm = exp_gpu(-CFP[3]*P[7]); 	// exp(-bval*d)
