				...
				T &gradYLocal)
{
  ...
  if(!CFP[4]){ //HR
    ...
    Dperpendicular = Dparallel*(1-P[14]);
  }
			 ...
			 T* FixP) // Fixed Parameters for each voxel
{
  ...
  T Fib_1;
  if(!CFP[4]){ //HR
    Fib_1 = (1-P[1])*ExtraCell + P[1]*IntraCell;
  }else{  // LR
    Fib_1 = (1-P[14])*ExtraCell + P[14]*IntraCell;
  }
  ...
  T Fib_2;
  if(!CFP[4]){ // HR
    Fib_2 =  (1-P[1])*ExtraCell + P[1]*IntraCell;
  }else{ // LR
    Fib_2 =  (1-P[14])*ExtraCell + P[14]*IntraCell;
  }

  T Anisoterm, pred_signal;

  if(!CFP[4]){ // HR
    Anisoterm = (1-P[2])*Fib_1 + P[2]*Fib_2;
    pred_signal=FixP[0]*(P[0]*isoterm + (1-P[0])*Anisoterm);
  }else{ // LR
    Anisoterm = (1-P[15])*Fib_1 + P[15]*Fib_2;
    pred_signal=FixP[0]*(P[13]*isoterm + (1-P[13])*Anisoterm);
  }
  return pred_signal;
}
			       ...
			       T* derivatives) // Derivative respect each model estimated parameter
{
  ...
  derivatives[14]=NUMERICAL(14);
  derivatives[15]=NUMERICAL(15);
  derivatives[16]=NUMERICAL(16);
  derivatives[17]=NUMERICAL(17);
  derivatives[18]=NUMERICAL(18);
  derivatives[19]=NUMERICAL(19);
}
