...
{  
  ...
  if(!CFP[4]){ //HR
    ...
    fanningdir[0] = -cos_gpu(P[11])*sin_gpu(P[5]) - cos_gpu(P[4])*cos(P[5])*sin_gpu(P[11]);
    fanningdir[1] = cos_gpu(P[5])*cos_gpu(P[11]) - cos_gpu(P[4])*sin_gpu(P[5])*sin_gpu(P[11]);
    fanningdir[2] = sin_gpu(P[4])*sin_gpu(P[11]);
  }
