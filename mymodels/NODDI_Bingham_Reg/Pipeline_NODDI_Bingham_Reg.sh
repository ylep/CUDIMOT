...
Usage() {
    echo ""
    echo "Usage: Pipeline_NODDI_Bingham_Reg.sh <subject_directory> [options]"
    echo ""
...
# Specify Priors
PriorsFile=$modelsdir/NODDI_Bingham/modelpriors
PriorsFile_Reg=$modelsdir/$modelname/modelpriors
...
outputdir=$PathStep1
Step1Opts=$opts" --outputdir=$outputdir --partsdir=$partsdir --priors=$PriorsFile --FixP=$FixPFile --gridSearch=$GridFile --init_params=$InitializationFile --fixed=4,5"

postproc=`${modelsdir}/utils/jobs_wrapper.sh $PathStep1 $initProcess NODDI_Bingham GS $njobs $Step1Opts`
...
echo $PathStep1/Param_6_samples >> $InitializationFile #LR_psi


partsdir=${subjdir}.${modelname}/diff_parts
outputdir=${subjdir}.${modelname}
ModelOpts=$opts_Reg" --outputdir=$outputdir --partsdir=$partsdir --priors=$PriorsFile_Reg --FixP=$FixPFile --init_params=$InitializationFile $lastStepModelOpts"
